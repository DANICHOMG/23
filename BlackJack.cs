...
namespace client.Core
{
    class BlackJack : Script
    {
        ...
        public static void Bet(Player player)
        {
            ...
            if (Main.Players[player].Money < 5000)
            {
            ...
            Trigger.ClientEventInRange(player.Position, 20, "bet_blackjack", player.GetData<int>("BJ_TABLE"), player.GetData<int>("BJ_SEAT"), player.Handle);
        ...
        public static void Exit(Player player)
        {
            ...
            Trigger.ClientEventInRange(GET_TABLE_CORD(player.GetData<int>("BJ_TABLE")), 20, "exit_table", player.Handle);
            ExitPlayer(player);
            //Trigger.PlayerEvent(player, "seat_to_blackjack_table", 1, 0);
        }
        ...
        public static void StartDealing(object table)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    BlackJackTables[tb] = new List<List<int>>()
                {
                    new List<int>{ 0, 0, 0, 0, 0, 0, 0, },
                    new List<int>{ 0, 0, 0, 0, 0, 0, 0, },
                    new List<int>{ 0, 0, 0, 0, 0, 0, 0, },
